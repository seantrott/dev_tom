...
```{r}
df_diff <- df_summ %>%
  arrange(model_shorthand, tokens_seen_numeric_mod) %>%
  group_by(model_shorthand) %>%
  mutate(
    acc_diff = c(NA, diff(mean_accuracy)),  # difference between consecutive points
    tokens_diff = c(NA, diff(tokens_seen_numeric_mod)),  # optional, could normalize slope
    slope = acc_diff / tokens_diff  # slope per token if you like
  ) %>%
  ungroup()

onsets <- df_diff %>%
  group_by(model_shorthand) %>%
  slice_max(acc_diff, n = 1) %>%
  ungroup() %>%
  dplyr::select(model_shorthand, tokens_seen_numeric_mod, acc_diff, slope)
onsets
