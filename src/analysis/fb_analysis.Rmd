...
ggplot(tmp, aes(x = n_params_approx, y = mean_accuracy)) +
  ...
  labs(
    ...
    subtitle = "Colored circles highlight accuracy at final pretraining step",
    x = "Number of Parameters (log scale)",
    y = "Accuracy"
  ) +
  theme_minimal() + 
...
# PAPER FIGURE: Plot of Olmo stage1 and Pythias (without 14M)
min_val <- df_summ_fb %>%
  filter(step > 0) %>%
  filter(stage == "stage1" | is.na(stage)) %>%
  filter(model_shorthand != "Pythia 14m") %>%
  pull(tokens_seen_numeric_mod) %>%
  min(na.rm = TRUE)
...
df_summ_fb %>%
  ...
  geom_hline(yintercept = .5, linetype = "dashed",
             size = 1, alpha = .8) +
  scale_x_log10(breaks = sort(unique(c(min_val, scales::log_breaks()(range(df_summ_fb$tokens_seen_numeric_mod))))),
...
# Filter for only stage1 olmo data and pythia models greater than 14m (e.g. exclude olmo stage2)
olmo_and_pythia <- df_all_models_fb %>%
  filter(stage == "stage1" | is.na(stage)) %>%
  filter(model_shorthand != "Pythia 14m")

# Z-score the variables that will go into the statistical model
olmo_and_pythia <- olmo_and_pythia %>%
  mutate(z_nparams = scale(n_params_approx),
         z_tokensseen = scale(tokens_seen_numeric_mod))
...
mod_full = glmer(data = olmo_and_pythia,
                 correct ~ condition + knowledge_cue + 
                   ...
                   log10(n_params_approx) +
                   log10(tokens_seen_numeric_mod) +
                   log10(n_params_approx) * log10(tokens_seen_numeric_mod) +
                   (1 | start) +
                   (1 | model_shorthand),
                 family = binomial())
summary(mod_full)

# with z-scaled variables, without interaction between stimulus properties
mod_full = glmer(data = olmo_and_pythia,
                 correct ~ condition + knowledge_cue + 
                   z_nparams +
                   z_tokensseen +
                   z_nparams * z_tokensseen +
                   (1 | start) +
                   (1 | model_shorthand),
                 family = binomial())
summary(mod_full)

# with z-scaled variables, AND interaction terms
mod_full = glmer(data = olmo_and_pythia,
                 correct ~ condition + knowledge_cue + 
                   condition * knowledge_cue + 
                   z_nparams +
...
df_coef %>%
  ...
  coord_flip() +
  scale_x_discrete(labels = c("conditionTrue Belief" = "Knowledge State (True Belief)",
                              "conditionTrue Belief:knowledge_cueImplicit" = "Knowledge State:Knowledge Cue",
                               "z_tokensseen" = "# Tokens Seen (z-scored)",
                               "z_nparams" = "# Parameters (z-scored)",
                               "knowledge_cueImplicit" = "Knowledge Cue (Implicit)",
                              "z_nparams:z_tokensseen" = "# Parameters:# Tokens Seen")) +
  labs(
    x = NULL, y = "Coefficient Estimate",
  ) +
  theme_minimal(base_size = 12)
max(olmo_and_pythia$step)

# Stage 1: Most models do get higher accuracy for true belief than false belief  
stage1_mean_acc_by_knowledge_state <- olmo_and_pythia %>%
  group_by(model_shorthand, condition) %>%
  summarise(accuracy = mean(correct)) 
print(stage1_mean_acc_by_knowledge_state)

stage1_mean_acc_by_stimproperties <- olmo_and_pythia %>%
  group_by(model_shorthand, condition, knowledge_cue) %>%
  summarise(accuracy = mean(correct))
print(stage1_mean_acc_by_stimproperties)

olmo_and_pythia %>%
  group_by(model_shorthand, condition, knowledge_cue) %>%
  mutate(accuracy = mean(correct)) %>%
  filter(model_shorthand == "Olmo 2 13b") %>%
  ggplot(aes(x = model_shorthand, y = accuracy, fill = knowledge_cue)) +
  geom_boxplot() +
  facet_wrap(~ condition) +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  labs(title = "Olmo 2 13b Mean FB Task Accuracy Across Pretraining")


# Stage 1 & Stage 2: Stage 2 improves mean accuracies in False Belief (for both Explicit and Implicit), and improves True Belief ONLY in implicit condition, but worsens performance in the explicit condition--Olmo13b EVEN MORE SENSITIVE to knowledge cue when you include Stage 2
df_all_models_fb %>%
  group_by(model_shorthand, condition) %>%
  summarise(accuracy = mean(correct)) 
...
ggcoef_model(mod_full, significance = 0.001)  # handles everything automatically
...
## False Belief Performance for Olmo 2 13b, by Knowledge State and Cue
(To do: run LRT at each time step for each model.)

```{r,echo=FALSE, warning=FALSE, message=FALSE}

df_summ = df_all_models_fb %>%
  group_by(model_shorthand, step,
...
df_summ %>%
  ...
  labs(color = "Knowledge State",       
       linetype = "Knowledge Cue") 
...
df_summ %>%
  ...
  labs(color = "Knowledge State",       
       ...
       x = "Number of Tokens Seen (log scale)",
       y = "Accuracy",
       title = "False Belief Task Performance by Stimulus Properties") 
...
## APPENDIX: companion figure, shows log odds
df_summ %>%
  filter(model_shorthand == "Olmo 2 13b") %>%
  filter(step > 1) %>%
  ggplot(aes(x = tokens_seen_numeric_mod,
...
## Finding Number of Tokens Seen for median accuracy
```{r,echo=FALSE, warning=FALSE, message=FALSE}
...
df_summ_fb_item = df_all_models_fb %>%
  mutate(start_end = paste(start, "-", end)) %>%
  group_by(start_end, model_path, model_shorthand,
           step, tokens_seen_numeric, model_family, stage) %>%
  summarise(mean_accuracy = mean(correct)) %>%
  mutate(tokens_seen_numeric_mod = tokens_seen_numeric + 1)
...
df_wide <- df_summ_fb_item %>%
  filter(tokens_seen_numeric_mod > 1000,
         model_shorthand == "Olmo 2 13b",
         stage == "stage1" | is.na(stage)) %>%
  select(tokens_seen_numeric_mod, start_end, mean_accuracy) %>%
  pivot_wider(names_from = start_end, values_from = mean_accuracy)

# Calculate correlations
cor_matrix <- df_wide %>%
  ungroup() %>%
  select(-model_path, -model_shorthand,
         -step, -tokens_seen_numeric, -model_family, -tokens_seen_numeric_mod) %>%
  cor(use = "pairwise.complete.obs")

# Visualize
library(ggcorrplot)
ggcorrplot(cor_matrix, title = "Item Trajectory Correlations")

# Get upper triangle (excluding diagonal)
upper_tri <- cor_matrix[upper.tri(cor_matrix)]
mean_cor <- mean(upper_tri)
summary(upper_tri)
hist(upper_tri)
print(paste("Average correlation:", round(mean_cor, 3)))


### Averaging across items with error bar
df_summ_item_avg = df_summ_fb_item %>%
  filter(tokens_seen_numeric_mod > 1000,
         model_shorthand == "Olmo 2 13b",
         stage == "stage1" | is.na(stage)) %>%
  group_by(tokens_seen_numeric_mod) %>%
  summarise(
    mean_acc = mean(mean_accuracy),
    sd_acc = sd(mean_accuracy),
    se_acc = sd_acc / sqrt(n())
  )

### Showing all item trajectories
df_summ_fb_item %>%
  filter(tokens_seen_numeric_mod > 1000,
         model_shorthand == "Olmo 2 13b",
         stage == "stage1" | is.na(stage)) %>%
  ggplot(aes(x = tokens_seen_numeric_mod, y = mean_accuracy)) +
  geom_line(size = .8, alpha = .8, color = "steelblue") +
  geom_hline(yintercept = .5, linetype = "dashed", alpha = 0.5) +
  geom_ribbon(data = df_summ_item_avg,
              aes(x = tokens_seen_numeric_mod, 
                  ymin = mean_acc - se_acc, 
                  ymax = mean_acc + se_acc),
              inherit.aes = FALSE,  # <- Add this line
              alpha = 0.3, fill = "red") +
  geom_line(data = df_summ_item_avg, aes(x= tokens_seen_numeric_mod, 
                                         y = mean_acc),
            size = 1.5,
            alpha = .5, color = "red") +
  scale_x_log10() + # labels = scales::comma_format()) +
  facet_wrap(~start_end, ncol = 4) +
  labs(x = "Tokens Seen", y = "Accuracy") +
  theme_minimal() +
  theme(text = element_text(size = 10))
```

## Pythia 12b max accuracy

```{r,echo=FALSE, warning=FALSE, message=FALSE}
# Find the tokens seen for Pythia12 max accuracy

# Filter for Pythia 12b  data
pythia_data <- df_summ_fb[df_summ_fb$model_shorthand == "Pythia 12b", ]

# Calculate the median accuracy for this model
max_accuracy_across_training <- max(pythia_data$mean_accuracy)

# Display results
print(paste("Max accuracy:", max_accuracy_across_training))

```

## Olmo 2 1b max accuracy
```{r,echo=FALSE, warning=FALSE, message=FALSE}
# Find the tokens seen for Olmo 2 1b max accuracy

# Filter for Olmo 2 1b  data
olmo_data <- df_summ_fb[df_summ_fb$model_shorthand == "Olmo 2 1b", ]

# Calculate the median accuracy for this model
max_accuracy_across_training <- max(olmo_data$mean_accuracy)
...
## Find Number of Tokens Seen for max accuracy in Olmo2 13b
```{r,echo=FALSE, warning=FALSE, message=FALSE}
# 
...
# Find the row(s) where accuracy is closest to the median
# Calculate absolute difference from median
olmo_data$diff_from_max <- abs(olmo_data$mean_accuracy - max_accuracy_across_training)

# Find the tokens_seen value where accuracy is closest to max
closest_row <- olmo_data[which.min(olmo_data$diff_from_max), ]
tokens_at_max <- closest_row$tokens_seen_numeric_mod
...
## Find Number of Tokens seen for median, 75th percentile, and max accuracies in Olmo2 7b
```{r,echo=FALSE, warning=FALSE, message=FALSE}
# Find the tokens seen for Olmo 2 7b for given accuracies

# Filter for Olmo 2 target size data
...
df_attn_check = df_attn_check %>%
  ...
  mutate(q_label = case_when(
    question_id == 1 ~ "At start of story, AGENT put ITEM in {LOC}",
    question_id == 2 ~ "At end of story, ITEM in {LOC}",
    question_id == 3 ~ "Person who put ITEM in START was {AGENT}",
    question_id == 4 ~ "Person who moved ITEM to END was {AGENT}"
  ))
...
df_summ_attn %>%
  ...
  theme(text = element_text(size = 15),
        legend.position="bottom") +
  facet_wrap(~reorder(q_label, question_id), labeller = label_wrap_gen(width = 40))
...
df_attn_check %>% 
  ...
  geom_density_ridges2(aes(height = ..density..), 
                       ...
                       alpha = .6,
                       stat="density") +
  labs(x = "Log Odds (Correct vs. Incorrect)",
       y = "",
       fill = "") +
  theme_minimal() +
...
df_for_gam <- fb_and_all_attn # save this for gam fits later!
...
fb_and_all_attn %>%
  ...
  scale_x_log10() + 
  scale_y_continuous(limits = c(0.25, 0.75)) + 
  labs(title = "Trajectory of Situation Model & False Belief Task Performance",
       x = "log Tokens Seen",
       y = "Accuracy",
       color = "",
       shape = "") +
...
```{r}
## ====== ====== ====== ====== ====== ====== ====== ====== ====== ======
## ====== TODO: get attention head information scores  ======
## ====== ====== ====== ====== ====== ====== ====== ====== ====== ======
# tells us: how informative is a given attention head's scoring over ROIs--are 
# their patterns uniform (uninformative) or pretty indicative of a given ROI?
# use Fenton information scores for principal cells
...
```{r}
## ====== ====== ====== ====== ====== ====== ====== ====== ====== ======
## ====== TODO: avg Q1 and Q2 vs. avg Q3 and Q4  ======
## ====== ====== ====== ====== ====== ====== ====== ====== ====== ======
# these are differentially useful to know ?
```

```{r,echo=FALSE, warning=FALSE, message=FALSE}
## ====== ====== ====== ====== ====== ====== ====== ====== ====== ======
## ====== Find checkpoints where performance increases or tanks ======
## ====== ====== ====== ====== ====== ====== ====== ====== ====== ======
# Aggregate to get accuracy per checkpoint (helps GAM fit) for just Olmo stage1 & pythia
...
# Fit a GAM for each model and task type separately
fits <- tmp %>%
  group_by(model_shorthand, task_type) %>%
  nest() %>%
  mutate(
    gam_fit = map(data, ~ gam(mean_accuracy ~ s(tokens_seen_numeric_mod, k = 10), 
                              data = .))
  )

# Find hingepoints (or, elbow points)
elbow_points <- tmp %>%
  group_by(model_shorthand, task_type) %>%
  group_split() %>%
  set_names(map_chr(., ~ paste(unique(.$model_shorthand), 
                               unique(.$task_type), 
                               sep = " - "))) %>%
  map(~ gam(mean_accuracy ~ s(tokens_seen_numeric_mod, k = 10), data = .)) %>%
  imap_dfr(function(model, name) {
    ...
    model_data <- tmp %>% 
      filter(model_shorthand == model_name, task_type == task_name) %>%
      filter(!is.na(tokens_seen_numeric_mod))
    
    if (nrow(model_data) == 0) {
      return(NULL)
    }
    
    token_range <- seq(min(model_data$tokens_seen_numeric_mod),
                       max(model_data$tokens_seen_numeric_mod),
                       length.out = 1000)
    
    predictions <- predict(model, 
                          newdata = data.frame(tokens_seen_numeric_mod = token_range))
    
    # Find peak first
    peak_idx <- which.max(predictions)
    
    # Only look for crossing after the peak
    after_peak <- predictions[(peak_idx + 1):length(predictions)]
    below_chance_after_peak <- which(after_peak < 0.5)
    
    if (length(below_chance_after_peak) == 0) {
      return(data.frame(
        model_shorthand = model_name,
        task_type = task_name,
        below_chance_tokens = NA,
        below_chance_accuracy = NA,
        peak_tokens = token_range[peak_idx],
        peak_accuracy = predictions[peak_idx],
        goes_below_chance = FALSE
      ))
    } else {
      first_below_idx <- peak_idx + below_chance_after_peak[1]
      
      return(data.frame(
        model_shorthand = model_name,
        task_type = task_name,
        below_chance_tokens = token_range[first_below_idx],
        below_chance_accuracy = predictions[first_below_idx],
        peak_tokens = token_range[peak_idx],
        peak_accuracy = predictions[peak_idx],
        goes_below_chance = TRUE
      ))
    }
  })
...
ggplot(tmp, aes(x = tokens_seen_numeric_mod, y = mean_accuracy)) +
  ...
  geom_smooth(method = "gam", 
              ...
              se = TRUE, 
              color = "blue") +
  geom_vline(data = elbow_points,
             aes(xintercept = below_chance_tokens),
             color = "red", linetype = "dashed") +
  facet_grid(task_type ~ model_shorthand, scales = "free") +
  scale_x_log10() +
  labs(title = "GAM Fits by LM and Task Type",
       x = "Log Tokens Seen",
       y = "Mean Accuracy") +
  theme_minimal()
...
plot_data %>%
  ...
  geom_hline(yintercept = .5, linetype = "dashed",
             size = 1, alpha = .8) +
  facet_wrap(~ reorder(q_label, question_id), ncol = 2, labeller = label_wrap_gen(width = 40)) +
  scale_x_log10() +
  ...
  labs(x = "Number of Tokens Seen (log scale)",
       color = "Task Type") +
...
### Lmer: Situation Modeling v. Mentalizing Over Pretraining
```{r}
### PAPER LMER: Poor-man's quantification of the visually appreciable differences in emergence of accuracies consistent with SM and Mentalizing over pretraining
mod_full = glmer(data = df_all,
                 correct ~ step +
                   ...
                   (1 | model_shorthand),
                 family = binomial())
